        ...
        <el-col :span="21">
          <el-card class="box-card">
            ...
            <el-divider content-position="left">Commands
             <el-input clearable v-model="sCode" size="small" @keyup.enter.native="searchCodes" placeholder="Keyword" style="width: 150px;"/>
            </el-divider>
            <el-tooltip v-for="o in codes" class="item" effect="dark" :content="o.v.code" placement="bottom-start">
              <el-button plain size="mini" type="success" @click="setCode(o.v)">{{o.v.name}}</el-button>
            </el-tooltip>
          </el-card>
        </el-col>
      </el-row>
...
<script>
  ...
  import { kvReadList,kvSearch} from '@/api/table'
	export default {
	  name: 'WebTerminal',
    ...
    created() {
      ...
      kvReadList({'b':'snippets','k':'code'}).then(response => {
        this.codes = response
      })
    },
	  methods:{
      ...
      getURL(){
        ...
        if (process.env.VUE_APP_BASE_API === '') {
          ...
          return protocol+process.env.VUE_APP_BASE_API.substring(7)
        }
	  }
	},
  }
